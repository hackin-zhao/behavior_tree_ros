...
#ifndef _BEHAVIOR_TREE__BT_CONVERSIONS_HPP_
#define _BEHAVIOR_TREE__BT_CONVERSIONS_HPP_
...
/**
 * @brief Parse XML string to geometry_msgs::Point
...
inline geometry_msgs::Point convertFromString(const StringView key)
{
  ...
  if (parts.size() != 3) {
    ROS_ERROR("invalid number of fields for point attribute)");
  } else {
...
/**
 * @brief Parse XML string to geometry_msgs::Quaternion
 * @param key XML string
...
inline geometry_msgs::Quaternion convertFromString(const StringView key)
{
  ...
  auto parts = BT::splitString(key, ';');
  if (parts.size() != 4) {
    ROS_ERROR("invalid number of fields for orientation attribute)");
  } else {
...
inline geometry_msgs::PoseStamped convertFromString(const StringView key)
{
  ...
  auto parts = BT::splitString(key, ';');
  if (parts.size() != 9) {
    ROS_ERROR("invalid number of fields for PoseStamped attribute)");
  } else {
...
#endif  // _BEHAVIOR_TREE__BT_CONVERSIONS_HPP_
